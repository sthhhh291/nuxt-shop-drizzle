...
<template>
        ...
        <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded">
          <div class="flex items-center justify-between">
            ...
            <span class="text-lg font-bold text-green-600">
              {{ formatCurrency((newLabor.hours || 0) * (newLabor.rate || 0)) }}
            </span>
          </div>
        </div>
      </div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          <UButton
            ...
            :loading="addingLabor"
            :disabled="
              !newLabor.description || !newLabor.hours || !newLabor.rate
            "
            @click="addLabor">
            Add Labor
          </UButton>
      ...
      <div class="space-y-4">
        ...
        <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded">
          <div class="flex items-center justify-between">
            ...
            <span class="text-lg font-bold text-green-600">
              {{
                formatCurrency((newPart.quantity || 0) * (newPart.price || 0))
              }}
            </span>
          </div>
        </div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          <UButton
            ...
            :loading="addingPart"
            :disabled="
              !newPart.part_name || !newPart.quantity || !newPart.price
            "
            @click="addPart">
            Add Part
          </UButton>
  ...
  <!-- <UModal v-model="showAddOilModal">
    <UCard>
      ...
      <div class="space-y-4">
        <UFormGroup label="Oil Type" required>
          <USelect
            v-model="newOil.oil_type"
            :options="oilTypes"
            placeholder="Select oil type..." />
        </UFormGroup>

        <UFormGroup label="Brand" required>
          <UInput v-model="newOil.brand" placeholder="Enter brand name..." />
        </UFormGroup>

        <div class="grid grid-cols-2 gap-4">
          <UFormGroup label="Quantity (quarts)" required>
            <UInput
              v-model.number="newOil.quantity"
              type="number"
              step="0.25"
              min="0"
              placeholder="0.00" />
          ...
          <UFormGroup label="Price per Quart ($)" required>
            <UInput
              v-model.number="newOil.price_per_quart"
              type="number"
              step="0.01"
              min="0"
              placeholder="0.00" />
        ...
        <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded">
          <div class="flex items-center justify-between">
            ...
            <span class="text-lg font-bold text-green-600">
              {{
                formatCurrency(
                  (newOil.quantity || 0) * (newOil.price_per_quart || 0)
                )
              }}
            </span>
          </div>
