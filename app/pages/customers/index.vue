...
// Computed pagination data for better type safety
const paginationData = computed(() => {
  return (customersData.value as any)?.pagination;
});

// Function to generate pagination links for UPagination
...
<template>
  <UContainer class="py-8">
    ...
    <div
      ...
      <div>
        ...
        <p class="text-gray-600 dark:text-gray-400 mt-1">
          ...
          <span v-if="paginationData?.totalCount" class="text-sm">
    ...
    <UCard v-if="(customersData as any)?.customers?.length > 0">
      ...
      <div
        class="divide-y divide-gray-200 dark:divide-gray-700 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          v-for="customer in (customersData as any).customers"
          :key="customer.id"
          class="py-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
          <NuxtLink :to="`/customers/${customer.id}`" class="block group">
    ...
    <!-- Pagination -->
    <div v-if="paginationData?.totalPages > 1" class="mt-6">
